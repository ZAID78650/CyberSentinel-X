...
import { Activity, ArrowLeft, Boxes, Dna, GitBranch, Lock, Radar, Siren, Zap } from "lucide-react";
...
export default function CampaignDetail() {
  ...
  const [commit, setCommit] = useState<CampaignCommit | null>(null);
    ...
    <div className="space-y-4">
      ...
      {tab === "Evidence & Blockchain" && (
        <Card
          ...
          subtitle="Tamper-evident chain of custody — hashes, Merkle roots, verification"
          actions={
            hasRole("ADMIN") || hasRole("SECURITY_ANALYST") ? (
              <button className="btn-primary" disabled={committing} onClick={async () => {
                setCommitting(true);
                try {
                  const res = await api.post<CampaignCommit>(`/evidence/campaign/${id}/commit`);
                  setCommit(res.data);
                  success("Campaign evidence committed", `Block #${res.data.block_index} anchored ${res.data.evidence_count} records with Merkle root ${res.data.merkle_root.slice(0, 12)}…`);
                } catch (err) {
                  toastError("Commit failed", (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "No evidence for this campaign yet.");
                } finally {
                  setCommitting(false);
                }
              }}>
                <Lock className="h-4 w-4" /> {commit ? "Re-commit campaign evidence" : "Commit campaign evidence"}
              </button>
            ) : undefined
          }
        >
          {evidence.isLoading && !evidence.data ? <Skeleton className="h-48" /> : !evidence.data || evidence.data.items.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {commit && (
                <div className="mt-3 rounded-lg border border-cyber-green/30 bg-cyber-green/5 p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <Lock className="h-4 w-4 text-cyber-green" />
                    <span className="badge border border-cyber-green/40 bg-cyber-green/10 text-cyber-green">BLOCKCHAIN VERIFIED</span>
                    <span className="font-mono text-xs text-slate-300">block #{commit.block_index} · {commit.evidence_count} evidence records</span>
                  </div>
                  <p className="mt-2 font-mono text-[11px] text-cyber-green">Merkle root {commit.merkle_root}</p>
                  <p className="font-mono text-[10px] text-slate-600">block hash {commit.block_hash}</p>
                </div>
              )}
