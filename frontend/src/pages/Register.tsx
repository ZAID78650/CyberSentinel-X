...
import { Loader2, ShieldCheck, UserPlus, Zap } from "lucide-react";
import { HeroLogo } from "../components/Logo";
import { useAuth } from "../contexts/AuthContext";
import { useToast } from "../components/ui/Toast";
import { getErrorMessage, isDemoMode } from "../services/api";
...
export default function Register() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      success("Account created", demoMode
        ? "Welcome to CyberSentinel X (Demo Mode)."
        : "Welcome to CyberSentinel X."
      );
      navigate("/dashboard");
    } catch (err) {
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10" style={{ background: "var(--surface)" }}>
      ...
      <div className="relative z-10 w-full max-w-lg">
          ...
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-cyber-yellow/30 bg-cyber-yellow/10 px-4 py-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-cyber-yellow/70">
                Account will be created locally — no backend server required
              </p>
            </div>
          </div>
        )}
        ...
        <div className="glass relative overflow-hidden p-8 shadow-[0_0_60px_rgba(56,189,248,0.12)]">
          ...
          <h2 className="flex items-center gap-2 text-xl font-bold" style={{ color: "var(--on-surface)" }}>
            <UserPlus className="h-5 w-5 text-electric-400" /> Create analyst account
          </h2>
          <p className="mt-1 text-sm" style={{ color: "var(--on-surface-faint)" }}>Join the CyberSentinel X security operations center</p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid gap-4 sm:grid-cols-2">
            ...
            <label className="flex items-center gap-2 text-xs sm:col-span-2" style={{ color: "var(--on-surface-muted)" }}>
